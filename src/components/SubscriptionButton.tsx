...
// ============================================================
// Assinatura do workspace (visão do superadmin).
// O botão e o modal são UM componente só — antes eles se comunicavam por uma
// variável global (window.__abrirAssinatura), e o modal nem chegava a ser
// renderizado na página: o botão não abria nada.
// ============================================================
...
export function SubscriptionButton({
  ...
  function abrir() {
    ...
    setMsg(null);
    start(async () => {
      const r = (await getSubscription(tenantId)) as any;
  ...
  function salvar(fd: FormData) {
    setMsg(null);
    start(async () => {
      const r = (await saveSubscription(tenantId, fd)) as any;
      if (r?.error) setMsg(r.error);
      else {
        setAberto(false);
  ...
  return (
    <>
      ...
      {aberto && (
        ...
        >
          <div
            className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-surface p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">Assinatura — {name}</h2>
